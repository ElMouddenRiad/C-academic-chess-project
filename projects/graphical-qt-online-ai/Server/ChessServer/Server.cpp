...
bool Server::SendString(int ID, string & _string)
{
	...
	int RetnCheck = send(it->second, _string.c_str(), static_cast<int>(_string.size()) + 1, 0); //Send string buffer
...
bool Server::GetString(int ID, string & _string)
{
	...
	int RetnCheck = recv(it->second, buffer, 512, NULL);
	if (RetnCheck <= 0)
		return false;
	if (RetnCheck >= 512)
		RetnCheck = 511;
...
bool Server::Processinfo(int ID)
{
	...
	if (json == NULL)
	{
		cout << "Received invalid JSON from user ID: " << ID << endl;
		return false;
	}
	...
	else
	{
		...
		else if (type == "CreateRoom")
		{
			...
			if (PlayerList[ID]->isFree())
			{
				...
				if (Username == NULL || Username->valuestring == NULL)
				{
					cJSON_Delete(json);
					return false;
				}
		...
		else if (type == "JoinRoomRequest")
		{
			...
			cJSON *Username;
			Username = cJSON_GetObjectItem(json, "User");
			if (Games_ID == NULL || Username == NULL || Username->valuestring == NULL)
			{
				cJSON_Delete(json);
				return false;
			}
