...
static bool SendJsonPayload(SOCKET connection, char *jsonText)
{
    if (jsonText == nullptr)
        return false;

    const int payloadSize = static_cast<int>(std::strlen(jsonText)) + 1;
    const int sent = send(connection, jsonText, payloadSize, 0);
    cJSON_free(jsonText);
...
bool gameLobby::sendMove(int FromX, int FromY, int ToX, int ToY)
{
    ...
    cJSON_Delete(Move);
    return SendJsonPayload(Connection, JsonToSend);
}
...
void gameLobby::I_wannaPlayAgain()
{
    ...
    cJSON_Delete(Mesg);
    SendJsonPayload(Connection, JsonToSend);
}
...
void gameLobby::CancelHost()
{
    ...
    char *JsonToSend = cJSON_Print(Mesg);   //make the json as char*
    cJSON_Delete(Mesg);
    if (SendJsonPayload(Connection, JsonToSend))
    {
        yourSide = -1;
        inRooms = false;
        waiting = false;
...
bool gameLobby::GetString()
{
    ...
    else if (type == "move")
    {
        if (inRooms)
        {
            ...
            if (FromX == NULL || FromY == NULL || ToX == NULL || ToY == NULL || Castling == NULL)
            {
                cJSON_Delete(json);
                return false;
            }
    ...
    else if (type == "System")
    {
        ...
        if (System_Info == NULL || System_Info->valuestring == NULL)
        {
            cJSON_Delete(json);
            return false;
        }
