...
public class BoardManager : MonoBehaviour
{
    ...
    private bool isApplyingStockfishMove = false;

    // Use this for initialization
    void Start()
    {
        Instance = this;
        stockfishManager = GameObject.FindObjectOfType<StockfishManager>();
        SpawnAllChessmans();
        EnPassantMove = new int[2] { -1, -1 };
        moveHistory.Clear();
        isApplyingStockfishMove = false;
    }
    ...
    private void MoveChessman(int x, int y)
    {
        if (selectedChessman == null)
        {
            // La pièce sélectionnée est nulle, sortir de la méthode ou afficher un message d'erreur
            return;
        }
        
        if (allowedMoves[x, y])
        {
            Chessman movedPiece = selectedChessman;
            int fromX = movedPiece.CurrentX;
            int fromY = movedPiece.CurrentY;
            ...
            RegisterMove(movedPiece, fromX, fromY, x, y);
            if (!isApplyingStockfishMove && stockfishManager != null)
                stockfishManager.MakeAIMove();
        }
    ...
    public string GetUciMoveHistory()
    {
        return string.Join(" ", moveHistory.ToArray());
    }

    public bool ApplyUciMove(string uciMove)
    {
        ...
        bool previousFlag = isApplyingStockfishMove;
        isApplyingStockfishMove = true;
        try
        {
            SelectChessman(fromX, fromY);
            if (selectedChessman == null)
                return false;
        ...
        {
            isApplyingStockfishMove = previousFlag;
        }
    }

    private void RegisterMove(Chessman movedPiece, int fromX, int fromY, int toX, int toY)
    {
        string move = ToUciFile(fromX).ToString() + (fromY + 1).ToString() + ToUciFile(toX).ToString() + (toY + 1).ToString();

        if (movedPiece is Pawn && (toY == 0 || toY == 7))
            move += "q";

        moveHistory.Add(move);
    }

    private static char ToUciFile(int x)
    {
        return (char)('a' + x);
    }
